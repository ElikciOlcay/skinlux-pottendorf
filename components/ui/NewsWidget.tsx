...
type WidgetVariant = "mothersDay" | "skinpen";

export default function NewsWidget() {
    const [isVisible, setIsVisible] = useState(false);
    const [variant, setVariant] = useState<WidgetVariant>("skinpen");

    useEffect(() => {
        setVariant(isMothersDayPromoActive() ? "mothersDay" : "skinpen");
                        ...
                        <div className="rounded-2xl border border-white/15 bg-black/80 backdrop-blur-md shadow-2xl overflow-hidden">
                            <div className="flex items-center justify-between px-3 py-2 border-b border-white/10">
                                <span className="text-[11px] tracking-[0.15em] uppercase text-white/70 font-light">
                                    {variant === "mothersDay" ? "Aktion" : "Aktuelles"}
                                </span>
                                <button
                            ...
                            {variant === "mothersDay" ? (
                                <MothersDayContent onDismiss={handleDismiss} />
                            ) : (
                                <SkinpenContent onDismiss={handleDismiss} />
                            )}
                        </div>
                    </motion.aside>
                </div>
...
function MothersDayContent({ onDismiss }: { onDismiss: () => void }) {
    return (
        <>
            <div className="p-3 flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                    ...
                    <p className="text-white/60 text-xs font-light mt-1 leading-snug">
                        Im ganzen Mai · Gültig für{" "}
...
function SkinpenContent({ onDismiss }: { onDismiss: () => void }) {
    return (
        <>
            <div className="p-3 flex items-center gap-3">
